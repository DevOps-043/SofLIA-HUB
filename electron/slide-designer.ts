...
async function generateSlideImage(
  ...
  try {
    ...
    console.warn(
      "[slide-designer] Image generation failed for prompt:",
      prompt,
      err,
    );
    return null;
  ...
  if (left.items.length > 0) {
    const bullets = left.items.map((b) => ({
      ...
      options: {
        ...
        fontFace: theme.fontBody,
        color: colors.text,
        bullet: { code: "2713", color: colors.accent },
        paraSpaceAfter: 6,
      },
    }));
  ...
  if (right.items.length > 0) {
    const bullets = right.items.map((b) => ({
      ...
      options: {
        ...
        fontFace: theme.fontBody,
        color: colors.text,
        bullet: { code: "2713", color: colors.accentAlt },
        paraSpaceAfter: 6,
      },
    }));
    slide.addText(bullets as any, {
  ...
  const flush = () => {
    ...
    if (isFirst) {
      slides.push({
        ...
        subtitle:
          currentBullets.length > 0 ? currentBullets.join(" • ") : undefined,
...
export async function createPremiumPresentation(
  ...
  const baseTheme =
    THEMES[themeName as keyof typeof THEMES] || THEMES["corporate-dark"];
  ...
  if (includeImages && genAI) {
    console.log(
      `[slide-designer] Generating images for ${slides.length} slides...`,
    );
    ...
    const successCount = Array.from(imageCache.values()).filter(
      (v) => v !== null,
    ).length;
    console.log(
      `[slide-designer] Generated ${successCount}/${slides.length} images successfully.`,
    );
  }
  ...
  console.log(
    `[slide-designer] Presentation saved: ${outputPath} (${slides.length} slides)`,
  );
  return outputPath;
}
