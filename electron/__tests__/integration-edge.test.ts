...
describe('Additional Integration Tests', () => {
  // INT-005: IPC channel namespace validation
...
describe('EDGE — Security and Validation', () => {
  // EDGE-SEC-001: Blocked tools in WhatsApp groups
  it('EDGE-SEC-001: dangerous tools are blocked in group chats', () => {
    const GROUP_BLOCKED = new Set([
      ...
      'open_application', 'kill_process', 'lock_session', 'shutdown_computer',
      'move_item',
    ]);

    const isBlocked = (toolName: string, isGroup: boolean) => {
  ...
  // EDGE-SEC-002: Payload sanitization removes __proto__
  it('EDGE-SEC-002: payload sanitization strips dangerous keys', () => {
    ...
    const malicious = { name: 'test', __proto__: { admin: true }, data: { constructor: 'evil' } };
    const sanitized = sanitizePayload(malicious);
  ...
  // EDGE-SEC-003: CSP injection present
...
describe('EDGE — Data Integrity', () => {
  // EDGE-DATA-001: Deduplication by ID
  ...
  // EDGE-DATA-002: Conversation limit enforcement
  it('EDGE-DATA-002: conversations are capped at MAX_CONVERSATIONS', () => {
    const MAX_CONVERSATIONS = 500;
    const conversations = Array.from({ length: 600 }, (_, i) => ({
      id: `conv-${i}`,
  ...
  // EDGE-DATA-003: Jaccard similarity for CRM deduplication
