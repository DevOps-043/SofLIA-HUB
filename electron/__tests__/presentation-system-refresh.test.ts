...
describe('actualizacion del motor de una presentacion persistente', () => {
  ...
  it('no crea archivos del runtime HTML en una presentacion React', async () => {
    const service = new SkillWorkspaceService(baseDir);
    const created = await service.createWorkspace({
      skillId: 'sistema:presentaciones',
      title: 'Deck React',
    ...
    expect(await refreshPresentationSystem(service, created.data.id)).toEqual({ ok: true });
    expect(await service.readFile(created.data.id, 'estilos/base.css')).toMatchObject({ ok: false });
    expect(await service.readFile(created.data.id, 'guion-base.js')).toMatchObject({ ok: false });
  });
