...
describe('WhatsApp Agent', () => {
  ...
  describe('Passive workflows', () => {
    it('detects natural scheduled requests and stores a passive workflow instead of sending them to Gemini', async () => {
      ...
      await agent.handleMessage(
        '123@s.whatsapp.net',
        '5215500000000',
        'Dame mis correos mas relevantes a las 8 am',
      );
      ...
      expect(waService.sendText).toHaveBeenCalledWith(
        '123@s.whatsapp.net',
        expect.stringContaining('workflow pasivo'),
      );
    ...
    it('passes skipConfirmations=true when executing scheduled passive tasks', async () => {
            ...
            candidates: [{
              content: {
                parts: [{
                  functionCall: {
                    name: 'gmail_send',
                    args: { to: 'a@b.com', subject: 'Hola', body: 'Texto' },
                  },
                }],
              },
            }],
      ...
      await agent.handleScheduledTaskTrigger(
        '123@s.whatsapp.net',
        '5215500000000',
        {
          id: 'task_1',
      ...
      expect(executeWhatsAppTools).toHaveBeenCalledWith(
        expect.any(Array),
        expect.objectContaining({ skipConfirmations: true }),
        '123@s.whatsapp.net',
        '5215500000000',
        false,
      );
  ...
  // --------------------------------------------------------------------------
  // WA-051: Action requests retry if model only promises future work
  // --------------------------------------------------------------------------
  describe('WA-051: action requests force execution instead of vague promises', () => {
    it('retries when the model says it will investigate but has not executed any tool yet', async () => {
      const waService = createMockWaService();
      const memory = createMockMemoryService();
      const knowledge = createMockKnowledgeService();
      const agent = new WhatsAppAgent(waService, 'test-key', memory, knowledge);
        ...
        responses: [
          {
            functionResponse: {
              name: 'web_search',
              response: { success: true, results: 'Capitulo 5 del CCNA' },
            },
          },
        ],
        bulkLabelsToVerify: null,
      ...
      mockSendMessage
        .mockResolvedValueOnce({
          response: {
            text: () => 'Voy a investigar en la web y te comparto el resultado.',
            candidates: [{ content: { parts: [{ text: 'Voy a investigar en la web y te comparto el resultado.' }] } }],
            functionCalls: () => null,
          },
        })
        .mockResolvedValueOnce({
            ...
            candidates: [{
              content: {
                parts: [{
                  functionCall: {
                    name: 'web_search',
                    args: { query: 'capitulo 5 cisco ccna titulo' },
                  },
                }],
              },
            }],
        ...
        .mockResolvedValueOnce({
          response: {
            text: () => 'El capitulo 5 cubre conceptos de switching.',
            candidates: [{ content: { parts: [{ text: 'El capitulo 5 cubre conceptos de switching.' }] } }],
            functionCalls: () => null,
          },
        });
      ...
      await agent.handleMessage(
        '123@s.whatsapp.net',
        '5215500000000',
        'Ayudame a investigar de que trata el capitulo 5 del CCNA',
      );

      expect(executeWhatsAppTools).toHaveBeenCalled();
      expect(waService.sendText).toHaveBeenCalledWith(
        '123@s.whatsapp.net',
        'El capitulo 5 cubre conceptos de switching.',
      );
    });
  });

  // --------------------------------------------------------------------------
  // WA-052: Generic help fallback must not answer substantive messages
  // --------------------------------------------------------------------------
  describe('WA-052: generic help response is retried for substantive messages', () => {
    it('retries when the model answers a non-greeting with a generic help prompt', async () => {
      ...
      mockSendMessage
        .mockResolvedValueOnce({
          response: {
            ...
            candidates: [{ content: { parts: [{ text: '¿En qué puedo ayudarte?' }] } }],
            functionCalls: () => null,
          },
        })
        .mockResolvedValueOnce({
          response: {
            text: () => 'Dime cuales errores viste y los reviso contigo.',
            candidates: [{ content: { parts: [{ text: 'Dime cuales errores viste y los reviso contigo.' }] } }],
            functionCalls: () => null,
          },
        });
      ...
      expect(waService.sendText).toHaveBeenCalledWith(
        '123@s.whatsapp.net',
        'Dime cuales errores viste y los reviso contigo.',
      );
    });
  });
