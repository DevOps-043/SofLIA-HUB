    ...
    <div className="rounded-xl border border-amber-500/20 bg-amber-500/8 p-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {detail.engine === 'automation' && detail.normalizedStatus === 'pending_approval' && (
          <>
            <button type="button" className="rounded-xl bg-accent hover:brightness-105 text-on-accent px-4 py-2 text-sm font-semibold transition disabled:opacity-40" onClick={() => void decide('case', 'approve-case', `Caso aprobado: ${detail.title}.`)} disabled={model.actionKey === 'approve-case'}>{model.actionKey === 'approve-case' ? 'Aprobando...' : 'Aprobar'}</button>
