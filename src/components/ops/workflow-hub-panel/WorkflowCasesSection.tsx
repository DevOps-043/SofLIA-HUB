...
export function WorkflowCasesSection({ controller }: Props) {
  ...
  return (
    <section className="px-6 pb-6 border-t border-border pt-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {cases.map((item) => (
              <button key={item.id} type="button" onClick={() => controller.setSelectedCaseId(controller.selectedCaseId === item.id ? null : item.id)}
                className={`w-full text-left rounded-xl border px-4 py-3 transition-all ${controller.selectedCaseId === item.id ? 'border-accent/30 bg-accent/10 shadow-md ring-1 ring-accent/10' : 'border-border hover:border-accent/20 bg-surface'}`}>
                ...
                <p className="text-[10px] text-secondary mt-1.5 truncate">{item.workflowName} - {formatDateTime(item.updatedAt)}</p>
              </button>
            ))}
          </div>
...
function EmptyCases() {
  return (
    <div className="rounded-2xl border border-dashed border-border px-4 py-10 text-center text-xs text-secondary">
