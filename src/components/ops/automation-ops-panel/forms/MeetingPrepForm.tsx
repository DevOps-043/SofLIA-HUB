...
export function MeetingPrepForm({ controller }: { controller: AutomationOpsController }) {
  const meeting = controller.forms.meeting;
  ...
  return (
    <div className="space-y-3">
      ...
      <input className={inputClass} type="date" value={meeting.date} onChange={(event) => meeting.setDate(event.target.value)} />
      <details className="rounded-xl border border-gray-200 dark:border-white/[0.06] bg-gray-50 dark:bg-white/[0.02] px-3 py-2">
        ...
        <div className="mt-3">
          <SpaceSelect spaces={controller.data.spaces} value={meeting.chatSpace} onChange={meeting.setChatSpace} emptyLabel="Sin Google Chat" />
        </div>
      </details>
      <button
        type="button"
        ...
        onClick={() => void controller.runner.runAction('run-meeting-prep', async () => {
          const result = await executeAutomationTemplate({ templateId: 'calendar_meeting_prep', requestedBy: `app:${controller.userId}`, input: { targetDate: meeting.date || undefined, gchatSpace: meeting.chatSpace || undefined } });
        ...
        disabled={controller.runner.actionKey === 'run-meeting-prep'}
      >
        {controller.runner.actionKey === 'run-meeting-prep' ? 'Preparando...' : 'Preparar reunion'}
      </button>
    </div>
  );
}
