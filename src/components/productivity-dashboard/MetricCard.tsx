...
export function MetricCard({ label, value, icon, trend, color = 'blue' }: MetricCardProps) {
  return (
    <div className={`relative overflow-hidden bg-surface border rounded-2xl p-6 transition-colors group/metric shadow-sm dark:shadow-none ${colorMap[color]}`}>
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          <div className={`p-3 rounded-xl ${iconColors[color]}`}>
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={icons[icon]} />
              {icon === 'camera' && <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />}
            </svg>
          </div>
          {trend !== 'neutral' && (
            <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${trend === 'up' ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'}`}>
        ...
        <p className="text-3xl font-semibold text-gray-900 dark:text-white truncate mt-1">{value}</p>
      </div>
    </div>
  );
}
