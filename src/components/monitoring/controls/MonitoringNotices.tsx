    ...
    <>
      {isRunning && calendarEventTitle && calendarAutoSessionId && (
        <div className="mt-4 flex items-center gap-2 px-3 py-2 bg-blue-500/5 border border-blue-500/10 rounded-xl">
          ...
          <p className="text-xs text-blue-500 dark:text-blue-400 font-medium truncate">Auto: {calendarEventTitle}</p>
        </div>
      )}

      {(error || persistError) && (
        <div className="mt-4 p-3 bg-danger/10 border border-danger/20 rounded-xl">
          <p className="text-xs text-danger font-medium text-center">{error || persistError}</p>
        </div>
      )}

      {(status?.diagnostics?.activeWinFailCount || 0) > 0 && (
        <div className="mt-4 flex items-center gap-2 px-3 py-2 bg-warning/5 border border-warning/15 rounded-xl">
          ...
          <span className="text-xs text-warning/80 font-medium">Interferencia en detección de ventana</span>
        </div>
      )}
    </>
  );
