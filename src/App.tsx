      ...
      >
        ...
        <nav className="flex-1 px-2 py-2 space-y-1 overflow-y-auto no-scrollbar">
          ...
          <button
            ...
            className={`w-full flex items-center ${isSidebarOpen ? 'gap-3 px-3' : 'justify-center px-0'} py-2 rounded-lg text-sm transition-colors text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-[#2A2B32] hover:text-gray-900 dark:hover:text-white`}
            title="WhatsApp"
          >
            <svg className="h-4.5 w-4.5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
            ...
            <>
              {isSidebarOpen && (
                <div className="pt-4 pb-1 px-3 flex items-center justify-between">
                  <span className="text-[11px] text-gray-400 dark:text-gray-500 uppercase tracking-widest font-bold">Proyectos</span>
                  <button 
                    onClick={(e) => { e.stopPropagation(); refreshIrisData(); }}
                ...
                return (
                  <div key={team.team_id} className="mb-0.5">
                    ...
                    >
                      ...
                      {isSidebarOpen && (
                        <>
                          <span className="flex-1 text-left truncate text-[13.5px] font-medium">{team.name}</span>
                    ...
                    {isTeamExpanded && isSidebarOpen && (
                      <div className="ml-5 mt-0.5 border-l border-gray-200 dark:border-white/5 space-y-0.5">
                        {teamProjects.length === 0 ? (
                          <p className="px-4 py-1.5 text-[11px] text-gray-400 dark:text-gray-500 italic font-light">Sin proyectos</p>
                        ) : (
                                ...
                                {isProjectExpanded && (
                                  <div className="ml-3 mt-1 space-y-0.5 border-l border-gray-200 dark:border-white/5">
                                    {projectIssues.length === 0 ? (
                                      <p className="px-4 py-1 text-[10px] text-gray-400 dark:text-gray-500 italic">Sin tareas</p>
                                    ) : (
                                      projectIssues.map(issue => {
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            />
                                            <span className="text-gray-500 dark:text-gray-600 shrink-0 tabular-nums">#{issue.issue_number}</span>
              ...
              {(() => {
                const projectsWithoutTeam = irisProjects.filter(p => !p.team_id || !irisTeams.some(t => t.team_id === p.team_id));
                  ...
                  <div className="mt-2 pl-1">
                    {isSidebarOpen && (
                      <div className="px-3 py-1 mb-1 opacity-50">
                        <span className="text-[9px] text-gray-400 dark:text-gray-500 uppercase tracking-widest font-bold">Proyectos Globales</span>
                      </div>
                    )}
                      ...
                      return (
                        <div key={project.project_id} className="mb-0.5">
                          ...
                          >
                            ...
                            {isSidebarOpen && (
                              <>
                                <span className="flex-1 text-left truncate font-medium">{project.project_name}</span>
                                ...
                                return (
                                  ...
                                  >
                                    <span
                                      ...
                                      style={{ backgroundColor: issueStatusColor }}
                                    />
                                    <span className="text-gray-500 dark:text-gray-600 shrink-0">#{issue.issue_number}</span>
                                    <span className="flex-1 text-left truncate">{issue.title}</span>
                                  </button>
                                );
                              })}
            ...
            >
              {sofiaContext?.currentOrganization?.brand_favicon_url ? (
                ...
                <div className="w-8 h-8 rounded-full bg-accent/20 flex items-center justify-center text-accent text-xs font-bold flex-shrink-0 overflow-hidden" title={displayName}>
                  {(sofiaContext?.user?.avatar_url || (sofiaContext?.user as any)?.profile_picture_url || user?.user_metadata?.avatar_url || (user?.user_metadata as any)?.profile_picture_url || (userSettings as any)?.profile_picture_url) ? (
                    <img 
                      src={sofiaContext?.user?.avatar_url || (sofiaContext?.user as any)?.profile_picture_url || user?.user_metadata?.avatar_url || (user?.user_metadata as any)?.profile_picture_url || (userSettings as any)?.profile_picture_url} 
                      alt="User" 
