...
export function purgeConversationFromAllPendingStates(conversationId: string): void {
  try {
    // Los identificadores se recolectan antes de escribir: `writePendingChatState`
    // puede eliminar la clave cuando el estado queda vacio, y eso desplaza los
    // indices de `localStorage` a mitad del recorrido. Con el recorrido directo
    // se saltaba una identidad, cuyo upsert pendiente volvia a crear en Supabase
    // la conversacion recien borrada.
