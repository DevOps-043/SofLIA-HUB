...
export const COMPUTER_PROCESS_TOOL_DECLARATIONS: GeminiFunctionDeclaration[] = [
  ...
  { name: 'open_application', description: 'Abre un archivo o aplicacion. Acepta ruta completa o nombre comun de la app y en Windows intenta resolver ejecutables instalados, accesos directos y alias del sistema. SOLO abre: no interactua con la app. Si el usuario pidio ademas una accion dentro de la app (reproducir, escribir, configurar, hacer click), continua con use_computer hasta completar TODA la tarea.', parameters: objectParams({ path: stringProp('Ruta completa o nombre comun de la aplicacion o archivo.') }, ['path']) },
  { name: 'open_url', description: 'Abre una URL en el navegador predeterminado. SOLO abre la pagina: no interactua con ella. Si el usuario pidio ademas una accion dentro del sitio (reproducir una cancion, dar click, llenar un formulario, publicar), NO te detengas aqui: continua con use_computer describiendo la tarea completa, y solo reporta exito cuando outcome.estado sea "completada".', parameters: objectParams({ url: stringProp('URL completa, incluyendo https://.') }, ['url']) },
