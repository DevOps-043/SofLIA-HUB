...
export function PresentationPlayerApp() {
  ...
  const move = useCallback((delta: number) => {
    ...
    setIndex((current) => {
      ...
      if (next !== current) setDirection(Math.sign(delta) || 1);
      return next;
    });
  }, [deck]);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      else if (event.key === 'End' && deck) setIndex(deck.slides.length - 1);
    };
  ...
  return (
    <main className="fixed inset-0 overflow-hidden bg-[#071119] text-white" aria-label={deck.meta.titulo}>
      ...
      >
        <AnimatePresence initial={false} custom={{ direction, continuity: slide.movimiento.continuidad }} mode="popLayout">
          <SlideFrame
            key={slide.id}
            slide={slide}
            index={safeIndex}
...
function SlideContent(props: { slide: PresentationSlide; reducedMotion: boolean; assetBase: string; embeddedAssets?: Record<string, string> }) {
  ...
  if (slide.tipo === 'portada') {
    return <motion.div variants={group} initial="hidden" animate="visible" className="runtime-pad grid h-full grid-cols-[1.05fr_.95fr] items-center gap-20 pb-40">
      <div className="relative z-20">{header}{slide.subtitulo ? <motion.p {...common} className="runtime-lead mt-9">{slide.subtitulo}</motion.p> : null}</div>
      <motion.div {...common} className="relative h-[720px] overflow-hidden rounded-[54px] bg-[color-mix(in_srgb,var(--marca-color-primario)_10%,white)] shadow-2xl">
  ...
  if (slide.tipo === 'declaracion') {
    return <motion.div variants={group} initial="hidden" animate="visible" className="runtime-pad flex h-full flex-col justify-center pb-36">
      <div className="max-w-[1500px]">{header}{slide.texto ? <motion.p {...common} className="runtime-lead mt-10 max-w-[1100px]">{slide.texto}</motion.p> : null}</div>
      {slide.acento ? <motion.div {...common} className="mt-16 inline-flex w-fit border-l-[10px] border-[var(--marca-color-acento)] pl-7 text-[34px] font-bold">{slide.acento}</motion.div> : null}
    </motion.div>;
  }
  ...
  if (slide.tipo === 'division') {
    ...
    const visual = <motion.div {...common} className="h-[690px] overflow-hidden rounded-[44px] bg-white/50 shadow-xl"><RuntimeImage image={slide.imagen} assetBase={props.assetBase} embeddedAssets={props.embeddedAssets} /></motion.div>;
  ...
  if (slide.tipo === 'comparacion') {
    return <motion.div variants={group} initial="hidden" animate="visible" className="runtime-pad h-full pb-36 pt-24">{header}<div className="mt-14 grid grid-cols-2 gap-8"><CompareBlock block={slide.izquierda} item={item} /><CompareBlock block={slide.derecha} item={item} primary /></div></motion.div>;
  }

  if (slide.tipo === 'proceso') {
    return <motion.div variants={group} initial="hidden" animate="visible" className="runtime-pad h-full pb-36 pt-24">{header}{slide.introduccion ? <motion.p {...common} className="runtime-lead mt-5">{slide.introduccion}</motion.p> : null}<div className="relative mt-16 grid gap-6" style={{ gridTemplateColumns: `repeat(${slide.pasos.length}, minmax(0,1fr))` }}><div className="absolute left-[8%] right-[8%] top-11 h-1 bg-[var(--marca-color-secundario)]/30" />{slide.pasos.map((paso, i) => <motion.div {...common} key={paso.titulo} className="relative z-10"><span className="grid h-24 w-24 place-items-center rounded-full border-[5px] border-[var(--marca-color-primario)] bg-[var(--marca-color-fondo)] text-[30px] font-black">{String(i + 1).padStart(2, '0')}</span><h2 className="mt-8 text-[34px] font-black leading-tight">{paso.titulo}</h2>{paso.texto ? <p className="mt-4 text-[24px] leading-snug opacity-70">{paso.texto}</p> : null}</motion.div>)}</div></motion.div>;
  }

  if (slide.tipo === 'metricas') {
    return <motion.div variants={group} initial="hidden" animate="visible" className="runtime-pad h-full pb-36 pt-24">{header}{slide.introduccion ? <motion.p {...common} className="runtime-lead mt-5">{slide.introduccion}</motion.p> : null}<div className="mt-14 grid gap-6" style={{ gridTemplateColumns: `repeat(${slide.metricas.length}, minmax(0,1fr))` }}>{slide.metricas.map((metric) => <motion.div {...common} key={metric.etiqueta} className="border-t-[8px] border-[var(--marca-color-acento)] bg-white/45 px-8 py-10"><strong className="block text-[76px] font-black tracking-[-.06em] text-[var(--marca-color-primario)]">{metric.valor}</strong><h2 className="mt-5 text-[30px] font-black">{metric.etiqueta}</h2>{metric.detalle ? <p className="mt-3 text-[22px] leading-snug opacity-65">{metric.detalle}</p> : null}</motion.div>)}</div></motion.div>;
  }
...
function CompareBlock(props: { block: { etiqueta?: string; titulo: string; texto?: string; puntos?: string[] }; item: ReturnType<typeof contentVariants>; primary?: boolean }) {
  return <motion.section variants={props.item} className={`min-h-[430px] border-t-[8px] p-10 ${props.primary ? 'border-[var(--marca-color-acento)] bg-[var(--marca-color-primario)] text-white' : 'border-current/25 bg-white/45'}`}>
...
function RuntimeImage(props: { image: { src: string; alt: string; ajuste: 'cubrir' | 'contener'; posicion: string }; assetBase: string; embeddedAssets?: Record<string, string> }) {
  ...
  return <img src={source} alt={props.image.alt} className={`h-full w-full ${props.image.ajuste === 'cubrir' ? 'object-cover' : 'object-contain'} ${positionClass(props.image.posicion)}`} />;
}

function Atmosphere({ emphasis }: { emphasis: PresentationSlide['movimiento']['enfasis'] }) {
  return <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
    <div className={`absolute -left-40 -top-52 h-[680px] w-[680px] rounded-full bg-[var(--marca-color-secundario)] opacity-15 blur-[90px] ${emphasis === 'pulso' ? 'runtime-breathe' : ''}`} />
...
function sceneStates(continuity: PresentationSlide['movimiento']['continuidad'], direction: number, reduced: boolean) {
  ...
  if (continuity === 'zoom') return { initial: { opacity: 0, scale: direction > 0 ? 1.08 : .92 }, animate: { opacity: 1, scale: 1 }, exit: { opacity: 0, scale: direction > 0 ? .94 : 1.06 } };
  if (continuity === 'empuje') return { initial: { opacity: 0, x: direction * 220 }, animate: { opacity: 1, x: 0 }, exit: { opacity: 0, x: direction * -180 } };
  if (continuity === 'corte') return { initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 } };
  return { initial: { opacity: 0, y: direction * 80, scale: .985 }, animate: { opacity: 1, y: 0, scale: 1 }, exit: { opacity: 0, y: direction * -60, scale: 1.012 } };
}
