...
- `sql/db-health-audit-queries.sql`: solo lectura. Ejecutar primero en Lia, IRIS y SOFIA/Learning para ver conexiones, scans, indices, RLS y FKs sin indice.
- `sql/lia-performance-hardening.sql`: indices para conversaciones, mensajes, shares, workspace sources y monitoring.
- `sql/iris-performance-hardening.sql`: indices para equipos, proyectos, issues, catalogos, miembros y Meeting Ops.
- `sql/sofia-learning-performance-hardening.sql`: indices para login SOFIA, organizaciones/equipos, Learning y CourseGen/CourseEngine inbox.
- `sql/security-rls-hardening.sql`: script idempotente para reemplazar politicas heredadas `USING (true)` por RLS owner-based en Lia Monitoring/OAuth y en IRIS Meeting Ops existentes.
...
- RLS debe aplicarse con `sql/security-rls-hardening.sql` en instancias existentes despues de confirmar que `auth.uid()` coincide con `user_id`/`owner_user_id`.
