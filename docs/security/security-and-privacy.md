...
| Credencial de sesion en main | solo el *refresh token*, cifrado con `safeStorage`; si el sistema no ofrece cifrado NO se persiste (no se degrada a texto plano); no se registra, no vuelve al renderer por ningun canal y se borra al cerrar sesion | `electron/main/hub-session-store.ts`, `electron/main/hub-session.ts`, `electron/auth-state-handlers.ts` |
