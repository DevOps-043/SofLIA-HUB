...
export function AvailableUpdateCard(props: AvailableUpdateCardProps) {
  const { state, availableVersion, notes, progress, onDownload, onInstall } = props
  const isVisible = ['available', 'downloading', 'downloaded'].includes(state)

  if (!isVisible || !availableVersion) return null
  ...
  return (
    <div className="p-4 rounded-2xl bg-accent/5 border border-accent/15 space-y-4">
      ...
      {state === 'available' && <ActionButton label="Descargar actualizaciÃ³n" onClick={onDownload} tone="accent" />}
      {state === 'downloaded' && <ActionButton label="Reiniciar para actualizar" onClick={onInstall} tone="success" />}
    </div>
  )
}
...
function ActionButton({ label, onClick, tone }: { label: string; onClick: () => void; tone: 'accent' | 'success' }) {
