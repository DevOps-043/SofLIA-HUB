...
export function StatusNoticeCards({ state, checking, currentVersion, error, statusError }: StatusNoticeCardsProps) {
  return (
    <>
      ...
      {(state === 'error' || error) && (
        <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/15 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-red-400/70 mt-0.5">{error || statusError || 'No se pudo conectar al servidor de actualizaciones'}</p>
          </div>
        </div>
      )}
    </>
