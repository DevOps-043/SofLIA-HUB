...
| RF-036 | abrir una ventana orbe flotante con dictado, TTS, hide/wake y estado conversacional | la ventana independiente consume APIs `orb:*`; las respuestas se sintetizan con la voz `ELEVENLABS_VOICE_ID` y `eleven_turbo_v2_5` por defecto desde main, se reproducen en orden y mantienen texto/error visible si falta configuración o permiso | `electron/main/orb-window-controller.ts`, `electron/elevenlabs-tts.ts`, `electron/orb-tts.ts`, `src/components/orb/` |
