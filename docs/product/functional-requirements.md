...
| RF-039 | ofrecer un navegador integrado con sesion persistente aislada que usuario y agente puedan observar y controlar sobre la misma pagina | la vista permite navegacion manual; `use_computer` la abre/reutiliza; protocolos y permisos se gobiernan | `electron/integrated-browser/`, `src/components/browser/IntegratedBrowserPanel.tsx` |
