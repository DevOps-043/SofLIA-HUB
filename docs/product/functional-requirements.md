...
| RF-039 | ofrecer un navegador lateral ajustable con sesion e historial persistentes, credenciales cifradas y extensiones compatibles que usuario y agente puedan observar sobre la misma pagina | convive con el chat o lo cubre al expandirse; `use_computer` la abre/reutiliza; secretos no salen de main; extensiones requieren aprobacion | `electron/integrated-browser/`, `src/components/browser/BrowserWorkspaceLayout.tsx` |
