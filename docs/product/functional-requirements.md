...
| RF-039 | ofrecer un navegador integrado con chat u Orbe flotante, pestañas, sesión e historial persistentes, favoritos locales, credenciales cifradas y extensiones compatibles que usuario y agente puedan observar sobre la misma página | hasta 500 pestañas lógicas comparten la sesión con máximo 8 vistas vivas y suspensión LRU; hasta cuatro pestañas pueden moverse a ventanas nativas separadas sin recarga y dentro del mismo presupuesto; popups HTTP(S) quedan internos; dos pestañas pueden verse divididas o superpuestas y el foco define el objetivo del agente; el chat compacto permite cambiar modelo/razonamiento y crear, buscar o activar conversaciones mediante popovers accesibles, se alinea bajo la barra, se mueve, redimensiona, minimiza o cambia por la Orbe general; el modelo seleccionado conserva la orquestación, puede usar búsqueda web, URL Context, DOM saneado y navegación determinista sin Computer Use, y `use_computer` delega sólo la actuación visual necesaria al Gemini 3.6 Flash fijo; el compositor conserva alineación vertical simétrica; la fila secundaria se pliega e integra favoritos y accesos a extensiones; la dirección sugiere historial en un menú flotante; main conserva solo la última captura + DOM saneado de la pestaña activa, usa una captura pasiva de máximo 1024 px con cadencia base de diez segundos y cuatro segundos de calma, cede durante Computer Use, no invoca al modelo por sí solo y permite pausa visible; cada turno contextual solicita evidencia puntual y resuelve mensajes, personas o recursos visibles aun sin verbos de visión; un enlace público se investiga primero mediante búsqueda/URL Context y puede abrirse directamente en la misma sesión; Computer Use queda para clics, escritura, scroll, formularios, autenticación o fallback dinámico; redirecciones de subframes OAuth/2FA no generan errores globales, pero protocolos no permitidos del frame principal se bloquean; valores de formularios, contenido editable, contraseñas, secretos y rutas no salen de main; extensiones muestran permisos completos, requieren aprobación explícita y permiten reintento | `electron/integrated-browser/`, `electron/desktop-agent/task-entrypoint.ts`, `electron/desktop-agent/task-budget.ts`, `src/components/browser/BrowserWorkspaceLayout.tsx`, `src/components/browser/BrowserConversationMenu.tsx`, `src/components/browser/IntegratedBrowserPanel.tsx`, `src/components/browser/BrowserQuickAccessBar.tsx`, `src/services/gemini-chat/browser-grounding-intent.ts`, `src/services/gemini-chat/integrated-browser-tools.ts` |
