...
RF-037 incorpora zoom por pestaña en Electron 43, gestor con desbloqueo Windows
y bloqueo automático, sugerencias revisadas en formularios SPA y redirecciones
SSO sin compartir credenciales entre orígenes, comandos de voz humanos para
pestañas/tareas, memoria semántica opt-in y derivación local de formularios
sensibles. Límites y datos enviados se declaran en la
[plataforma](../architecture/integrated-browser-platform.md); no son paridad
con Proton Pass, un proveedor de passkeys ni aprobación del rollout.

RF-037 también permite recuperar desde soporte los ajustes locales dañados de
permisos, privacidad y agente, con respaldo compatible y confirmación nativa.
Recuperar retira concesiones/excepciones y conserva bloqueos; no restaura claves,
dispositivos revocados ni reglas empresariales. La UI comunica cancelación/error
sin anunciar éxito por un acuse incompleto o de otro perfil.
También recupera atajos con IDs/revisión nuevos y configuración sync con
transferencia desactivada, previa confirmación nativa; no ejecuta instrucciones
ni restaura checkpoints, dispositivos revocados o claves.
...
| RF-032 | listar/ejecutar plantillas y aprobar runs de automatizacion | cada run expone estado, detalle y decisiones | `electron/workspace-automation/` |
| RF-032b | programar Skills pasivas y entregarlas por los canales elegidos | la regla declara canales; un canal caido no impide los demas y el fallo no cancela la programacion | `electron/passive-skills/`, `electron/task-scheduler.ts` |
| RF-032c | activar o desactivar cada Skill por canal (Computadora, WhatsApp, Telegram) | la eleccion acota lo que declara el catalogo y nunca lo amplia; su ausencia no retira ninguna | `public.user_skill_channels`, `src/shared/skills/channels.ts` |
| RF-033 | emitir un resumen diario programado por WhatsApp con el estado del sistema y el contexto del dia | el envio ocurre solo si el servicio esta habilitado y tiene numero destino y API key; un fallo del modelo cae al resumen de respaldo sin cancelar la programacion | `electron/daily-briefing-service.ts`, `electron/daily-briefing/` |
