...
| DEC-013 | El navegador compartido usa `WebContentsView` acotadas con una partición propia, no `<webview>`, iframe ni perfil externo. | **Confirmada en OpenSpec:** permite pestañas y dos vistas reales con control main sin exponer Node/IPC ni mezclar cookies privilegiadas; el foco selecciona el objetivo del agente. | `openspec/changes/evolve-integrated-browser-workspace/design.md`, `electron/integrated-browser/service.ts` |
