...
- [ ] 8.7 Añadir migraciones versionadas, respaldos y rollback de stores del navegador. Sesión y bóveda v1→v2 verificadas; recuperación HITL de marcadores y bóveda implementada, DPAPI real y borrado de copias verificados. Historial/bitácora/memoria crean DDL y versión en transacción con rollback y rechazan bases futuras, ajenas o v1 incompletas preservando bytes. Permisos, privacidad y política del agente ya tienen respaldo protegido por el SO y recuperación restrictiva con HITL, DTO cerrado y guardas de sesión/perfil/control, publicación directa de caché, flush y conservación del original dañado. Recuperar no devuelve concesiones/excepciones; restablecer permisos elimina todas sus copias locales. Se verificó DPAPI real en los tres stores. Atajos y configuración sync ya cuentan con copia previa y recuperación HITL: atajos con nuevos IDs/revisión y sin ejecución; sync con categorías vacías y sin red. Eliminar atajos o guardar pausa retira sus copias locales; versiones futuras/ámbito/cuotas se rechazan. Memoria semántica ya tiene respaldo SQLite vacío protegido y recuperación HITL sin vectores, fuentes ni activación; conserva el original dañado hasta la siguiente escritura válida. Probada con DPAPI real. Faltan respaldos/recuperación SQLite de historial/bitácora, checkpoints y diario de conflictos sync, y rollback integral; no restaurar dispositivos revocados mediante copias antiguas. No se declara cierre.

## 9. Documentación, verificación y cierre

Último corte: [atajos y configuración sync el 2026-09-12](reports/verification-shortcut-sync-recovery-2026-09-12.md).
Continúa la [recuperación restrictiva de ajustes](reports/verification-policy-recovery-2026-09-12.md) y avanza dos almacenes adicionales de 8.7 sin declarar cierre integral: **62/64 completas y 2 pendientes** (8.7 y 9.5).
...
- [x] 9.3 Ejecutar typecheck, lint de cambios, harness y validaciones documentales. Typecheck, lint incremental, docs:check, OpenSpec y diff check aprobados; harness incluido en verify:pr.
- [x] 9.4 Ejecutar `npm run verify:pr` y separar fallos preexistentes de regresiones. Todas las compuertas pasan hasta `skills:seed:check`; queda únicamente la discrepancia preexistente entre `database/lia/migrations/system-skills-catalog.sql` y `src/shared/skills/registry.ts`, sin cambios en esta fase.
- [ ] 9.5 Ejecutar smoke manual de navegación, descarga, perfiles, privacidad, agente y restauración en Windows. Smoke focalizado automatizado sobre Electron 43.4.0 real ejecutado; flujo completo UI/IPC, perfiles de producto, privacidad, agente e instalador pendientes. Antes del rollout de sync: despliegue autorizado y pruebas Lia Auth/RLS/revocación con dos equipos reales; los fixtures HTTP locales no satisfacen esa condición.
