...
- Fijar Computer Use en `gemini-3.8-flash` sin degradación silenciosa, conservando SofLIA, SofLIA Max, SofLIA Pro y SofLIA Lite en el selector conversacional.
...
- Retirar la compatibilidad especial de llamadas directas de Google Chat: SofLIA no crea, transfiere, reintenta ni observa reuniones. Las aperturas automáticas de la ruta exacta `meet.google.com/call` originadas por Gmail o Chat se cancelan sin crear pestaña, ventana o navegador externo. La gobernanza general de cámara, micrófono, pantalla y notificaciones permanece intacta para la navegación web ordinaria.
...
- Conservar el modelo y razonamiento elegidos como orquestador del turno aunque necesite Computer Use; solo el actuador visual interno queda fijado en `gemini-3.8-flash`.
...
No objetivos: Chrome Web Store o `.crx`, compatibilidad completa con Chrome, sincronización cloud, captura automática de contraseñas, lectura de valores escritos en formularios, vigilancia del escritorio, inferencia continua sin un turno del usuario o una tarea activa, autofill iniciado por el modelo, ejecución de extensiones sin aprobación o reimplementar la interfaz, señalización o SDP privados de Google Chat. Las ventanas separadas no incorporan un renderer de aplicación, perfil o proceso de agente adicional.
...
Afecta el layout de `AppContent`/`AppWorkspace`/`AppSidebar`, componentes de navegador y chat, estado de preferencias, `IntegratedBrowserService`, nuevos stores locales bajo `userData`, handlers y canales `integrated-browser:*`, preload/wrapper, mocks Electron y pruebas main/renderer. Fija temporalmente Electron `44.0.0-beta.3`, con Chromium 152, porque el smoke limpio sobre Electron 43.4.0/Chromium 150 volvió a detenerse antes de `CreateMeetingDevice`; el runtime conserva la versión real del motor, elimina únicamente las marcas Electron del User-Agent y no falsifica Client Hints, modifica SDP ni amplía permisos. Historial y metadata de extensiones se guardan en JSON local; las contraseñas se almacenan únicamente cifradas por el proveedor seguro del sistema operativo. El rollback permite volver a Electron 43.4.0 o desactivar el sidecar y sus gestores conservando el navegador base.
