...
nueve arquetipos cerrados, incluido un arquetipo de gráfica respaldado por
Recharts; Tailwind expresa el sistema espacial y Framer Motion ejecuta
transiciones y microinteracciones con un vocabulario acotado. HyperFrames se integra como doctrina —una sola
...
La vista previa conserva un contenedor medible durante el estado incompleto; al
aparecer `deck.json` no depende de un nuevo resize para calcular la escala. En
desarrollo, los módulos de Vite aceptan el origen opaco del iframe sandbox sin
ampliar sus permisos. El servidor loopback refleja unicamente ese origen opaco
y el origen exacto de Vite configurado para la ventana de reproduccion; incluye
`Vary: Origin` y omite CORS para cualquier otro origen.

La entrada React usa imports dinámicos mutuamente excluyentes. La ruta de
presentación no carga `App.tsx`, autenticación ni Supabase, porque esos clientes
acceden a `localStorage` y el origen opaco debe seguir rechazándolo. El bloque de
Recharts también se difiere hasta encontrar una diapositiva `grafica`.

### D14. Visuales de fuente materializados antes de la generación

La observación saneada del navegador ya identifica imágenes de contenido, pero
una URL dentro del contexto no garantiza que el modelo la descargue ni que el
deck pueda abrirse sin red. Cuando la Skill activa es `sistema:presentaciones`,
el renderer selecciona como máximo ocho visuales: adjuntos visuales del turno y
hasta cinco imágenes web, deduplicadas y priorizadas por texto alternativo y
área. Antes de llamar al proveedor las escribe o descarga mediante los canales
acotados del workspace, y entrega al modelo un manifiesto de rutas `assets/`.

Los documentos Office conservan la lectura estructurada completa del sidecar y
añaden, cuando está disponible, una captura de su vista actual. Así fotografías,
diagramas y gráficas visibles llegan como evidencia visual sin degradar el texto
ni las tablas. Cada fallo de imagen es aislado: el documento y los visuales que
sí se importaron siguen disponibles. El prompt establece la prioridad
`fuente -> visual generado complementario`; una imagen generada nunca recrea ni
reemplaza una gráfica o evidencia documental disponible.
